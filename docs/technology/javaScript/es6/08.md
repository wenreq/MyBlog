...
注意，只要函数调用时，扩展运算符才可以放在圆括号中，否则会报错。
...
下面是扩展运算符取代 `apply()` 方法的一个实际例子，应用 `Math.max()` 方法，简化求出一个数组数组最大元素的写法。

```js
// ES5 的写法
Math.max.apply(null, [14, 7, 3, 77]) // 77

// ES6 的写法
Math.max(...[14, 7, 3, 77]) // 7

// 等同于
Math.max(14, 7, 3, 77); // 77
```

上面代码中，由于 JavaScript 不提供求数值最大元素的函数，所以只能套用 `Math.max()` 函数，将数组转为一个参数序列，然后求最大值。有了扩展运算符以后，就可以直接用 `Math.max()` 了。
...
```js
// ES5 
new (Date.bind.apply(Date, [null, 2022, 04, 26]))
...
数组是复合的数据类型，直接复制的话，只是赋值了指向底层数据结构的指针，而不是克隆一个权限的数组。
...
`Array.from()`还可以接受一个函数作为**第二个参数**，作用类似于数组的`map()`方法，用来对每个元素进行处理，将处理后的值放入返回的数组。
...
这个方法的主要目的，是弥补数组构造函数`Array()`的不足。因为**参数个数的不同**，会导致`Array()`的行为有差异。
...
Array.of(true, 1, 'wen', {age: 28}) // [true, 1, 'wen', {age: 28}]
```
...
该方法的第二个参数表示搜索的其实位置，默认为 0。如果第二个参数为负数，则表示倒数的位置，如果这时它大于数组的长度（比如第二个参数为 -4，但数组长度为 3），则会重置为从 0 开始。
...
为了解决这个问题，现在有一个提案，为数组实例增加了`at()`方法，接受一个整数作为参数，返回对应位置的成员，支持负索引。这个方法不仅可用于数组，也可用于字符串和类型数组（TypedArray）。
