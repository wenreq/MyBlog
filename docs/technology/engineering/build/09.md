...
当 loader 进行编译时的 AST 操作均为 CPU 密集型任务，使用 JavaScript 性能低下，此时可采用高性能语言 rust 变写的 `swc`。
...
而当开启持久化缓存功能，最耗时的 AST 解析将能够从磁盘的缓存中获取，再次编译时无需再次进行解析 AST。
...
[thread-loader](https://github.com/webpack-contrib/thread-loader) 为官方推荐的开启多进程的 `loader`，可对 babel 解析 AST 时开启多线程处理，提高编译的性能。
