...
通过 `ln -s` 创建一个软连接，通过 `ln` 可以创建一个硬链接。
...
1. 软连接可理解为指向源文件的指针，它是单独的一个文件，仅仅只有几个字节，它拥有独立的 `inode`
...
那么不可避免地在 npm 或 yarn 中，`lodash@3.0.0` 会被多次安装，无疑造成了空间的浪费与诸多问题。
...
./node_modules/package-c/node_modules/lodash
./node_modules/package-d
./node_modules/package-d/node_modules/lodash
```
...
而在 pnpm 中，它改变了 npm/yarn 的目录结构，采用软连接的方式，避免了 `doppelgangers` 问题更加节省空间。
...
如此，依赖*软连接*的方式，可解决重复依赖安装（doppelgangers）的问题，**如果一个项目占用 1000 MB，那么使用 pnpm 可能仅占用 800 MB**

然而它除此之外，还有一个最大的好处，**如果一个项目占用 1000MB，传统方式十个项目占用 10000 MB，那么使用 pnpm 可能仅占用 3000 MB，**而它得益于硬链接。

再借用以上示例，`lodash@3.0.0` 与 `lodash@4.0.0` 会生成一个指向全局目录(~/.pnpm-store)的硬链接，如果新项目依赖二者，则可复用储存空间。
